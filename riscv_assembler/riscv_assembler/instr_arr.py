...
class _S(Instruction):
	...
	def immediate(imm, n):
		...
		if n == 1:
			...
			return mod_imm[:7] # Bokun: changed the above line to this
...
class _U(Instruction):
	...
	@staticmethod
	def immediate(imm):
		# Bokun: changed "int(imm) >> 12" to "int(imm)" and "013b" to "020b"
		return format(int(imm) & ((1 << 20) - 1), '020b') # Bokun: fixed for negative input 
...
class InstructionParser:
	...
	def JUMP(tk : str, line_num : int, code: list) -> int:
		...
		print(f"[JUMP] {tk} encountered, index: {index}, line_num: {line_num}, skip_labels: {skip_labels}")
...
class _R_parse(InstructionParser):
	...
	def organize(self, tokens):
		...
		print(f"[R_parse] {instr} occured, rs1={rs1}, rs2={rs2}, rd={rd}, tokens[{len(tokens)}]: {tokens}")
...
class _I_parse(InstructionParser):
	...
	def organize(self, tokens):
		...
		if instr == "jalr": # Bokun's note: For jalr only, the compiler does not handle "jalr, rd, imm(rs1) " format, use "jalr rd, rs1, imm" instead!
			if len(tokens) == 4+2:
				# rs1, imm, rd = reg_map[tokens[2]], super().JUMP(tokens[3], line_num, code), reg_map[tokens[1]]
				rs1, imm, rd = reg_map[tokens[2]], tokens[3], reg_map[tokens[1]] # Bokun: changed the above line to this (handle "jalr, rd, rs1, imm" format)
			else:
				rs1, imm, rd = reg_map[tokens[1]], 0, reg_map["x1"]
		elif instr == "lw" or instr == "lb" or instr == "lbu" or instr == "lhu" or instr == "lh": # Bokun, added all the ORs
			rs1, imm, rd = reg_map[tokens[3]], tokens[2], reg_map[tokens[1]]
			# print(f"[I_parse] original immediate: {imm}")
			# print(f"[I_parse] immediate to be joined: {_I.immediate(imm)}")
		elif instr == 'ld':
			rs1, imm, rd = reg_map[tokens[3]], tokens[2], reg_map[tokens[1]]
		else:
			rs1, imm, rd = reg_map[tokens[2]], tokens[3], reg_map[tokens[1]]
		print(f"[I_parse] {instr} occured, rs1={rs1}, imm={imm}, rd={rd}, tokens[{len(tokens)}]: {tokens}")
...
class _S_parse(InstructionParser):
	...
	def organize(self, tokens):
		...
		print(f"[S_parse] {instr} occured, rs1={rs1}, rs2={rs2}, imm={imm}, tokens[{len(tokens)}]: {tokens}")
...
class _SB_parse(InstructionParser):
	...
	def organize(self, tokens):
		...
		print(f"     |____[B_parse] {instr} occured, rs1={rs1}, rs2={rs2}, imm={imm}, tokens[{len(tokens)}]: {tokens}")
...
class _U_parse(InstructionParser):
	...
	def organize(self, tokens):
		# Bokun: changed the index order from 0, 1, 2 to 0, 2, 1
		# the original order seems like a bug as it throw errors
		instr, imm, rd = tokens[0], tokens[2], reg_map[tokens[1]]
		print(f"[U_parse] {instr} occured, imm={imm}, rd={rd}, tokens[{len(tokens)}]: {tokens}")
...
class _UJ_parse(InstructionParser):
	...
	def organize(self, tokens):
		...
		if len(tokens) == 5: # Bokun: changed from 3 to 5
			# token structure:  ['jal'(opcode), 'x0', 'label', 91(linenumber), [list of code lines as strings]]
			imm, rd = super().JUMP(tokens[2], line_num, code), reg_map[tokens[1]]
		else:
			imm, rd = super().JUMP(tokens[1], line_num, code), reg_map["x1"]
		print(f"     |____[UJ_parse] {instr} occured, imm={imm}, rd={rd}, tokens[{len(tokens)}]: {tokens}")
...
class _Pseudo_parse(InstructionParser):
	...
	def organize(self, tokens):
		...
		if instr == 'nop':
			...
			print(f"[Pseudo: {instr}] addi occured, rs1={rs1}, imm={imm}, rd={rd}, tokens[{len(tokens)}]: {tokens}")
			...
			print(f"[Pseudo: {instr}] xori occured, rs1={rs1}, imm={imm}, rd={rd}, tokens[{len(tokens)}]: {tokens}")
			return I("xori", rs1, imm, rd)
		elif instr == "neg":
			rs1, rs2, rd = reg_map["x0"], reg_map[tokens[2]], reg_map[tokens[1]]
			print(f"[Pseudo: {instr}] sub occured, rs1={rs1}, rs2={rs2}, rd={rd}, tokens[{len(tokens)}]: {tokens}")
			return R("sub", rs1, rs2, rd)
		elif instr == "j":
			imm = super().JUMP(tokens[1], line_num, code)
			print(f"     |____[Pseudo: {instr}] jal occured, imm={imm}, rd=x0, tokens[{len(tokens)}]: {tokens}")
			return UJ("jal", imm, "x0")
		elif instr == "li":
			# This is missing addi rd rd -273
			print(f"[Pseudo: {instr}] lui occured, imm={tokens[2]}, rd={reg_map[tokens[1]]}, tokens[{len(tokens)}]: {tokens}")
